# -*- coding: utf-8 -*-
"""
Global autoencoder workflow for spectral diversity analysis (50 cm).

Patched to handle inf / -inf / extreme reflectance values before scaling,
and includes debug checks before MinMaxScaler.
"""
...
from spectral_workflow_50cm import process_spectral_workflow
...
def clean_roi_for_autoencoder(roi_ref, wl, reflectance_limit=5.0):
    """
    Prepare ROI cube for the global autoencoder.

    Patch added:
    - convert inf / -inf to NaN
    - remove extreme reflectance values beyond reflectance_limit
    - ensure returned pixel matrix is finite
    """
    ...
    # Patch 1: convert all non-finite values to NaN
    roi_cube[~np.isfinite(roi_cube)] = np.nan

    # Mask bad atmospheric absorption regions
    roi_cube[:, :, (1350 <= wl_nm) & (wl_nm <= 1500)] = np.nan
    roi_cube[:, :, (1790 <= wl_nm) & (wl_nm <= 1995)] = np.nan
    roi_cube[:, :, (wl_nm > 2475)] = np.nan

    # Patch 2: remove unrealistic reflectance magnitudes
    roi_cube[np.abs(roi_cube) > reflectance_limit] = np.nan

    # Remove bands that are entirely invalid
    ...
    # Remove pixels containing NaN in any kept band
    valid_pixel_mask = ~np.any(np.isnan(roi_cube_clean), axis=2)
    valid_pixels = roi_cube_clean[valid_pixel_mask, :]

    # Final safety check
    valid_pixels = valid_pixels[np.all(np.isfinite(valid_pixels), axis=1)]
...
def fit_global_autoencoder(pixel_matrices,
    ...
    # Patch 3: second-stage finite filtering before scaling
    fit_pixels = fit_pixels[np.all(np.isfinite(fit_pixels), axis=1)]
    fit_pixels = fit_pixels[np.nanmax(np.abs(fit_pixels), axis=1) < 5.0]

    print('fit_pixels shape:', fit_pixels.shape)
    print('NaN count:', np.isnan(fit_pixels).sum())
    print('Inf count:', np.isinf(fit_pixels).sum())
    print('Max abs value:', np.nanmax(np.abs(fit_pixels)))
...
# Latent metrics
# ---------------------------------------------------------
def compute_latent_metrics(pixel_matrix, encoder, scaler, max_pixels_for_distance=5000, random_state=42):
    pixel_matrix = pixel_matrix[np.all(np.isfinite(pixel_matrix), axis=1)]
...
if __name__ == '__main__':
    ...
    data_dirs = [
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/3sp/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/5sp_with_abundance/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/7sp/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/10sp_v2/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/12sp/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/15sp_with_abundance/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/17sp/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/20sp/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/22sp/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/25sp_with_abundance/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/27sp/50cm',
        r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/different_species/30sp_with_abundance/50cm',
    ]

    output_dir = r'D:/Drive F 5-11-2024/A_BiosCape_Project3/Spectral_diversity/global_autoencoder_results_50cm'
